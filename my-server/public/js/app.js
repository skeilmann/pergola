...
let ajax_form = {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (typeof response.func !== 'undefined') {
                        let func = window[response.func]; if (typeof func === 'function') {
                            func(response);
                        }
                    }
                    ...
                    if (form_captcha && response.type === 'error') {
                        grecaptcha.reset();
                    }
    ...
    show_alert: function (alert_ob, alert_type, alert_msg) {
        if (alert_ob) {
            switch (alert_type) {
                case 'error': alert_type = 'danger';
                    break;
                case 'info': alert_type = 'primary';
                    break;
            }
            alert_ob.removeClass('alert-danger alert-success alert-primary alert-warning').addClass('alert-' + alert_type).html(alert_msg);
        ...
        let form_btn = form_ob.find('button[type="submit"], input[type="submit"]');
        if (form_btn.length < 1) {
            let form_id = form_ob.attr('id');
            form_btn = false;
        ...
        if (btn_ob) {
            switch (btn_state) {
                case 'loading': btn_ob.width(btn_ob.width()).attr({
                    'disabled': true, 'data-label': btn_ob.html()
                });
                    btn_ob.html('<i class="fas fa-spinner fa-spin"></i>');
                    break;
                case 'normal': btn_ob.removeAttr('style disabled');
                    btn_ob.html(btn_ob.data('label'));
                    break;
                case 'disabled': btn_ob.removeAttr('style');
                    btn_ob.html(btn_ob.data('label'));
                    break;
            }
        }
        ...
        $.each(alerts, function (field, message) {
            let field_ob = form_ob.find('[name="' + field + '"]');
            if (field_ob.length > 0) {
                let help_block_ob = field_ob.closest('.form-group').find('span.help-block');
